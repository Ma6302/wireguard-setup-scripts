...
select_menu_option() {
  ...
  read -rp "选择操作 [1-6]：" option || { echo; echo "已退出。"; exit 0; }
...
# 必要性：选项1/4会显示QR码，若不暂停，循环重印菜单会把QR码顶出屏幕。
...
select_client_to() {
  ...
  # 若用户未输入编号，取消本操作并返回主菜单
  [ -z "$client_num" ] && { echo; echo "已取消，返回主菜单。"; return 1; }
...
wgsetup() {
  ...
  # 分支7：已安装WireGuard（存在配置文件），进入管理菜单
    ...
    install_wg_show_names
    # [F10] 主菜单循环：每个操作完成后返回菜单，仅选择「6) 退出」
    ...
    while true; do
      ...
      3)
        ...
        ;;
      4)
        # 选项4：显示指定客户端的QR码（编号输入为空 = 取消，直接返回菜单）
        ...
        ;;
      5)
        # 选项5：卸载WireGuard（选N = 中止并返回菜单；确认卸载完成后脚本结束，
