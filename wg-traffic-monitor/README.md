...
- **自动更新（双组件）**：每天日报时同时比对 `wgmon` 与 `wg.sh` 的版本，**谁有新版本更新谁**，
  微信通知里写明"更新了哪个"（可开关）
- 交互菜单（v1.4.1 起**二级化**，与 wg.sh 同款交互：操作后回车返回、子菜单可返回上一级）
...
3) 发送测试消息
4) 参数设置（阈值 / 日报时间 / 快照间隔）     ─┐
5) 修改 Server酱 SendKey                      │
6) 重装定时任务                                │
7) 更新管理（手动更新 / 检查更新 / 自动更新开关）─┤
8) 卸载 wgmon（移除定时任务/快捷命令，可选删数据） │
0) 退出                                        │
                                              │
-------- 参数设置 --------          ←─────────┘
   当前：阈值 20 GB ｜ 日报 23:35 ｜ 快照间隔 30 分钟
1) 修改当日告警阈值（当前 20 GB，0=禁用）
2) 修改日报推送时间（当前北京时间 23:35）   ← 改完自动重装定时任务
3) 修改快照间隔（当前 30 分钟）             ← 改完自动重装定时任务
4) 返回上一级

-------- 更新管理 --------
   当前：wgmon v1.4.1 ｜ wg.sh v1.4.0 ｜ 自动更新 开（来源: Release 标签）
1) 手动更新（上传 wgmon.py.new / wg.sh.new 后执行，保留配置）
2) 检查并安装更新（从 GitHub 下载，两个组件各自判断）
3) 自动更新开关（当前 开）
4) 返回上一级
```

- 二级菜单里操作完成 → 按回车回**本二级菜单**；选 `4) 返回上一级` → 回主菜单（不再多点一次回车）
- `SendKey` 与「重装定时任务」留在主菜单（前者是敏感项、后者是维护动作，都不属于日常调参）
- 任意菜单下 `Ctrl+D`（stdin 关闭）= 安全退出，不会抛异常
...
- **自动**：`auto_update = true` 时每天日报推送后检查一次；**手动**：菜单 `7) 更新管理` → `2)`，或 `wgmon check-update`；
  开关：菜单 `7) 更新管理` → `3)`；`update_wgsh = false` 可单独关掉 wg.sh 的自动更新（`[update]` 段）。
