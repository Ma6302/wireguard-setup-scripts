...
#
# ============================================================
# 【修订版】基于「组网工具（解压使用）/wg-wg执行文件/wg.sh」
# 原作者：包崽同学（二改汉化）
# 优化维护：Ma6302（https://github.com/Ma6302）
# 修订日期：2026-09-17（初版）/ 2026-09-19（追加 F11）   原始版本行数：1730
# 脚本版本：1.4.0（C7）
#   —— 版本号同时写在仓库根 WGSH_VERSION，供 wgmon 自动更新比对（勿只改一处）
#
# 修订内容：
#   [F1] 顶部 DNS 块：原版第 8 行 `>/etc/systemd/resolved.conf` 是无条件
#        执行的（在任何 if 之前），每次运行脚本都会把该文件截断为 0 字节；
#        而写入逻辑被 ./wg.txt 门控，第二次运行时不会进入，于是配置被永久
#        清空且无人回填。已在真实服务器复现：
#           /etc/systemd/resolved.conf       size=0  mtime=09-16 18:08:31
#           /etc/systemd/resolved.conf.bak   size=0  mtime=09-16 10:04:40
#        → 改为：先在 wg.txt==1 门控内做真实备份（cp -a，且不覆盖已有备份），
#          再用 `>` 幂等写入（原版用 `>>`，重复执行会累加）。
#   [F2] check_args：DNS 兜底值 `233.5.5.5` 不是有效地址，改为 `223.5.5.5`；
#        可达性探测由域名（google.com，依赖 DNS 本身）改为纯 IP 1.1.1.1，
#        避免「DNS 故障」被误判成「无外网」。
#   [F3] select_dns：菜单只列 1~4，但 case 只匹配 1/2/3/7，选中
#        「4) 自定义DNS服务器」时没有任何分支命中，dns 会停留在字符串 "4"，
#        最终生成非法客户端配置 `DNS = 4`。真正能自定义的是 7，而菜单里没有 7。
#        → case 增加 4（保留 7 兼容旧编号）；校验范围 ^[1-7]$ 改为 ^[1-4]$；
#          并在函数末尾加兜底，任何非法取值一律回退为 223.5.5.5。
#   [F4] select_dns 自动模式：原为直接 `dns=2`，会覆盖 check_args 已按外网
#        可达性算出的结果，语义上也不再是菜单编号。
#        → 改为仅在当前取值非法时才兜底。
#   [F5] 顶部 DNS 块：为 /etc/resolv.conf 建软链前，先确认系统确实使用
#        systemd-resolved。原版无条件建链，在 RHEL/CentOS 系（无 resolved）
#        上会留下指向不存在文件的断链，直接打坏系统 DNS。
#   [F6] 顶部 DNS 块：原版 stop systemd-resolved 后从不重启，且未说明原因。
#        实测该操作的真实目的是释放 127.0.0.53:53，避免与 WireGuard 的
#        0.0.0.0:53 冲突（DNSStubListener=no 即可达成）。
#        → 保留 stop 语义等价的效果：写入 DNSStubListener=no 后 restart，
#          让解析服务继续可用，同时不占用 53。
#   [F7] 顶部 DNS 块第 6 步：放行规则原为写死的 `--dport 53`，一旦改用其它
#        端口就会漏放行（该规则本身也不持久，重启即失效）。已改为跟随
#        `--port` 参数；拿不到端口时交由安装阶段的 wg-iptables.service
#        （其中本就用 $port 变量）放行。注意：此处位于脚本顶部、parse_args
#        尚未执行，$port / $server_port 此刻都是空值，不能直接引用。
#   [F8] new_client 生成客户端配置时：原为无条件下发
#        `AllowedIPs = 0.0.0.0/0, ::/0`。服务端若无 IPv6 出口（本机如此：
#        轻量应用服务器产品不支持配置 IPv6 地址），隧道内的 ::/0 会把设备
#        全部 IPv6 流量吸进隧道后丢弃 → IPv6 黑洞，而本地原生 IPv6（校园网
#        免费直连）被白白浪费。已改为与同函数 Address / 服务端 AllowedIPs
#        相同的门控：仅当 wg0.conf 存在服务端 ULA `fddd:2c4:2c4:2c4::1` 时
#        才追加 `, ::/0`；否则客户端 IPv6 走本地原生链路。
#        注：不用 $ip6 —— 它只在首次安装时由 detect_ipv6 赋值，后续菜单
#        新增客户端时为空，会误判。
#   [F9] 交互体验增强：安装（或已安装系统再次运行本脚本）时写入
#        /etc/profile.d/wg-named.sh，为登录 shell 定义 wg() 包装函数。
#        交互式终端执行 `wg` / `wg show` 时，自动根据 wg0.conf 中的
#        "# BEGIN_PEER <名称>" 注释，为每个 peer 行附加客户端名称（青色）。
#        安全性设计：仅当 stdout 是终端时才附加名称；管道/重定向场景输出
#        与原版 wg 逐字节一致，不影响任何脚本解析。函数只对交互式登录
#        shell 生效，wg-quick / systemd / 其它脚本调用 /usr/bin/wg
#        完全不受影响。卸载 WireGuard 时该文件会被一并删除。
#   [F10] 菜单交互重构：原为「一次性菜单」——每完成一个操作就 exit，
#        想做第二件事必须重新运行脚本。已改为主菜单循环：
#          · 操作完成后「按回车返回主菜单」（QR 码不会被菜单刷屏顶掉）；
#          · 子操作（输名称/选编号）直接回车 = 取消并返回菜单（原为退出脚本）；
#          · 卸载确认选 N = 中止并返回菜单（原为退出脚本）；
#          · 仅「4) 退出」或确认卸载完成后才真正结束脚本。
#        连带修复：菜单 read 遇 EOF（Ctrl+D / stdin 关闭）原会死循环，
#        现 EOF 即正常退出；确认类 read 遇 EOF 视为「N」（安全默认）。
#   [F11] 菜单二级化（2026-09-19）：主菜单原有三个平级项「2) 列出所有已存在的
#        客户端 / 3) 删除指定客户端 / 4) 显示指定客户端的QR码」——三者进来都
#        要先列一遍客户端，彼此重复。已合并为单一入口「管理已有客户端」：
#        进入后先直接显示当前已有客户端清单，再提供
#          1) 列出所有已存在的客户端
#          2) 删除指定客户端
#          3) 显示指定客户端的QR码
#          4) 返回上一级
#        四项；子操作完成/取消均回到本二级菜单，选 4 才回主菜单。主菜单
#        由 6 项减为 4 项。命令行参数（--listclients / --removeclient /
#        --showclientqr）完全不受影响。
...
#        v1.4.3 修复：① 旧版（C8 前）升级上来的服务器，配置里是旧版写的 fq，
#        菜单显示「关闭」且"默认开启"落空 —— 现在首次运行新版时自动补开
#        （用户明确关闭过则不动）；② 开启完成提示修正为"已即时生效"
#        （原误写"重启后自动生效"，net_opt_on 实际立即应用 tc/sysctl）。
#
...
# 原版此处第 8 行有一句无条件的 `>/etc/systemd/resolved.conf`，
# 每次运行脚本都会清空该文件；第二次运行时 wg.txt 已为 2，
# 下面的写入分支不会进入，配置因此被永久抹掉。
# ─────────────────────────────────────────────────────────────
if [[ ! -f ./wg.txt ]]; then
  echo "1" >./wg.txt
fi
...
if [[ $(cat ./wg.txt) -eq 1 ]]; then
  # 1) 先判断系统是否真的使用 systemd-resolved（[F5]）
  has_resolved=0
  if [ -d /run/systemd/resolve ] || [ -e /etc/systemd/resolved.conf ] ||
    systemctl list-unit-files 2>/dev/null | grep -q '^systemd-resolved\.service'; then
    has_resolved=1
  fi

  if [ "$has_resolved" = 1 ]; then
    # 2) 真实备份：只在备份不存在时创建，绝不覆盖已存在的原始备份（[F1]）
    if [ ! -e /etc/systemd/resolved.conf.bak ]; then
      cp -a /etc/systemd/resolved.conf /etc/systemd/resolved.conf.bak &&
        echo "已备份原始 DNS 配置 >> /etc/systemd/resolved.conf.bak"
    else
      echo "备份已存在，跳过（原始备份保持不动）>> /etc/systemd/resolved.conf.bak"
    fi

    # 3) 用纯 IP 探测外网可达性，决定上游 DNS（[F2]）
    if ping -c1 -W2 1.1.1.1 &>/dev/null; then
      wg_upstream_dns="1.1.1.1"
      echo "当前服务器可以正常访问外网>>DNS配置1.1.1.1"
    else
      wg_upstream_dns="223.5.5.5"
      echo "当前服务器无法正常访问外网>>DNS配置223.5.5.5"
    fi

    # 4) 幂等写入（用 > 而非 >>，重复执行不会累加）（[F1]）
    #    DNSStubListener=no 是必须的：否则 systemd-resolved 会占用
    #    127.0.0.53:53，与 WireGuard 的 0.0.0.0:53 绑定冲突（[F6]）
    cat >/etc/systemd/resolved.conf <<__WG_CONF_EOF__
[Resolve]
DNS=${wg_upstream_dns}
DNSStubListener=no
__WG_CONF_EOF__

    # 5) 重建 resolv.conf 软链并让配置立即生效（[F5][F6]）
    #    systemd-resolved 的 RuntimeDirectoryPreserve=yes，服务停止期间
    #    /run/systemd/resolve/resolv.conf 仍保留，不会变成断链
    ln -sf /run/systemd/resolve/resolv.conf /etc/resolv.conf
    systemctl restart systemd-resolved 2>/dev/null ||
      systemctl start systemd-resolved 2>/dev/null || true
  else
    echo "未检测到 systemd-resolved，跳过系统 DNS 处理（/etc/resolv.conf 保持原样）"
  fi

  # 6) 放行 WireGuard 监听端口（[F7]：不再写死 53）
  #    此处位于脚本顶部，parse_args 尚未执行，$port / $server_port 均为空值，
  #    因此先扫描命令行里的 --port；拿不到就跳过 —— 安装阶段的
  #    wg-iptables.service 会用实际端口写入持久放行规则（见 create_firewall_rules）。
  wg_early_port=""
  for ((i = 1; i <= $#; i++)); do
    if [[ "${!i}" == "--port" ]]; then
      j=$((i + 1))
      [ "$j" -le "$#" ] && wg_early_port="${!j}"
      break
    fi
  done
  if [[ "$wg_early_port" =~ ^[0-9]+$ ]] &&
    [ "$wg_early_port" -ge 1 ] && [ "$wg_early_port" -le 65535 ]; then
    # 幂等：已存在相同规则时不重复插入
    iptables -w 5 -C INPUT -p udp --dport "$wg_early_port" -j ACCEPT 2>/dev/null ||
      iptables -w 5 -I INPUT -p udp --dport "$wg_early_port" -j ACCEPT
    echo "已放行 WireGuard 监听端口 UDP/$wg_early_port（与 --port 一致）"
  else
    echo "未通过 --port 指定端口，本步跳过；端口放行由 wg-iptables.service 按实际端口负责。"
  fi
...
remove_pkgs() {
  # [F9] 删除 wg 名称显示增强（/etc/profile.d/wg-named.sh）
  rm -f /etc/profile.d/wg-named.sh
  if [[ "$os" == "ubuntu" ]]; then
    (
      set -x
      # 删除WireGuard配置目录
...
select_dns() {
  if [ "$auto" = 0 ]; then
    ...
    echo "   3) 阿里云 DNS（223.5.5.5）"
    echo "   4) 自定义DNS服务器"
    read -rp "DNS服务器 [2]：" dns
    # 验证用户选择的合法性（空或1-4，与上面的菜单逐项对应）（[F3]）
    until [[ -z "$dns" || "$dns" =~ ^[1-4]$ ]]; do
      echo "$dns：选择无效。"
      read -rp "DNS服务器 [2]：" dns
    done
  else
    # 自动模式：沿用 check_args 已按外网可达性选定的 DNS（[F4]）
    # 原版此处直接 dns=2，会覆盖 check_args 的结果，且取值不再是菜单编号
    if ! check_ip "$dns"; then
      dns="223.5.5.5"
    fi
  fi
  # 根据选择配置DNS服务器
  ...
  1)
    ...
    ;;
  2 | "")
    # 海外公共 DNS（1.1.1.1）（[F3] 修正过时注释）
    dns="1.1.1.1"
    ;;
  3)
    # 阿里云 DNS（223.5.5.5, 223.6.6.6）（[F3] 修正过时注释）
    dns="223.5.5.5"
    ;;
  4 | 7)
    # 自定义DNS服务器（让用户输入）；保留 7 以兼容旧编号（[F3]）
    enter_custom_dns
    if [ -n "$dns2" ]; then
      dns="$dns1, $dns2"
    else
  ...
  # 兜底：绝不允许 dns 停留在裸编号、空值或非法地址，
  # 否则会生成 "DNS = 4" 之类的非法客户端配置（[F3]）
  if ! check_ip "${dns%%,*}"; then
    echo "警告：DNS 取值异常（$dns），已回退为 223.5.5.5"
    dns="223.5.5.5"
  fi
}
...
net_opt_menu() {
  ...
  if net_opt_enabled; then
    ...
    if [[ ! "$ans" =~ ^[nN]$ ]]; then
      net_opt_on
      echo
      echo "网络优化已开启（已即时生效，重启后依然有效）。"
      echo "当前队列：$(tc qdisc show dev "$nic" 2>/dev/null | head -1)"
    else
      echo
      echo "已取消，网络优化保持关闭。"
      return 1
...
# [F9] 安装 wg 交互输出增强：定义 wg() 包装函数，使交互式终端中
# 执行 wg / wg show 时，每个 peer 行显示对应的客户端名称。
# 名称来源：/etc/wireguard/wg0.conf 的 "# BEGIN_PEER <名称>" 注释块。
# 仅登录 shell 生效（/etc/profile.d/），stdout 非终端时输出与原版一致。
install_wg_show_names() {
  cat <<'WG_NAMED_EOF' >/etc/profile.d/wg-named.sh
# WireGuard peer 名称显示增强（由 wg-fixed.sh [F9] 安装，卸载时自动删除）
# 效果：交互式终端执行 wg / wg show 时，为每个 peer 附加客户端名称。
# 仅在标准输出为终端时生效；管道/重定向输出与原版 wg 逐字节一致。
wg() {
  local wg_conf="/etc/wireguard/wg0.conf"
  # 非终端输出（管道/重定向/脚本）或配置不可读时，原样透传
  if [ ! -t 1 ] || [ ! -r "$wg_conf" ]; then
    command wg "$@"
    return
  fi
  local out rc
  ...
  if [ -n "$out" ]; then
    printf '%s\n' "$out" | awk -v conf="$wg_conf" '
      BEGIN {
        ...
        while ((getline line < conf) > 0) {
          if (line ~ /^# BEGIN_PEER[ \t]/) {
            ...
            sub(/^# BEGIN_PEER[ \t]+/, "", name)
            sub(/[ \t\r]+$/, "", name)
          } else if (line ~ /^# END_PEER[ \t]/) {
            name = ""
          } else if (name != "" && line ~ /^PublicKey[ \t]*=/) {
            key = line
            sub(/^PublicKey[ \t]*=[ \t]*/, "", key)
            sub(/[ \t\r]+$/, "", key)
            names[key] = name
      ...
      $1 == "peer:" && length($2) == 44 && ($2 in names) {
        print $0 "  " ESC "[1;36m[" names[$2] "]" ESC "[0m"
        next
      }
      { print }
    '
...
WG_NAMED_EOF
  chmod 644 /etc/profile.d/wg-named.sh
}
...
finish_setup() {
  ...
  # [F9] 安装 wg 显示增强（wg / wg show 显示客户端名称）
  install_wg_show_names
  echo
...
# [F11] 客户端管理二级菜单：进入即显示已有客户端清单，再给出四项操作。
#       清单每次进入都重新统计，删除客户端后返回本菜单即可看到刷新结果。
select_client_menu_option() {
  echo
  echo "────────── 客户端管理 ──────────"
  num_of_clients=$(grep -c '^# BEGIN_PEER' "$WG_CONF")
  if [[ "$num_of_clients" -gt 0 ]]; then
    echo "   已有客户端（共 $num_of_clients 个）："
    show_clients | sed 's/^/      /'
  else
    echo "   已有客户端：无"
  fi
  ...
  echo "   4) 返回上一级"
  # 与主菜单同款 EOF 兜底
  read -rp "选择操作 [1-4]：" client_menu_option || { echo; echo "已退出。"; exit 0; }
  until [[ "$client_menu_option" =~ ^[1-4]$ ]]; do
    echo "$client_menu_option：选择无效。"
    read -rp "选择操作 [1-4]：" client_menu_option || { echo; echo "已退出。"; exit 0; }
  done
}

# [F10] 操作完成后暂停，按回车返回主菜单。
# 必要性：选项1会显示QR码，若不暂停，循环重印菜单会把QR码顶出屏幕。
# EOF（Ctrl+D / stdin 关闭）时直接退出脚本，避免死循环。
press_enter_to_menu() {
  echo
  read -rp "按回车键返回主菜单…" || { echo; echo "已退出。"; exit 0; }
}

# [F11] 二级菜单操作完成后暂停，按回车返回「客户端管理」菜单
press_enter_to_client_menu() {
  echo
  read -rp "按回车键返回客户端管理菜单…" || { echo; echo "已退出。"; exit 0; }
}

# [F11] 「管理已有客户端」二级循环：承接原主菜单 2/3/4 三项功能。
#       子操作完成或取消（空输入）都回到本菜单；只有选 4「返回上一级」才回主菜单。
manage_clients() {
  while true; do
    select_client_menu_option
    case "$client_menu_option" in
    1)
      # 二级选项1：列出所有客户端
      print_check_clients
      if check_clients menu; then
        echo
        show_clients
        print_client_total
      fi
      ...
      ;;
    2)
      # 二级选项2：删除指定客户端（编号输入为空 = 取消，直接回本菜单）
      if ! check_clients menu || ! select_client_to "删除"; then
        continue
      fi
      ...
      ;;
    3)
      # 二级选项3：显示指定客户端的QR码（编号输入为空 = 取消，直接回本菜单）
      if ! check_clients menu || ! select_client_to "显示QR码"; then
        continue
      fi
      echo
      get_export_dir
      if check_client_conf menu; then
        show_client_qr_code
        print_client_conf
      fi
      press_enter_to_client_menu
...
# [F10] 空输入（直接回车）= 取消本操作并返回主菜单（返回码1），不再退出脚本
enter_client_name() {
  echo
  echo "请为新客户端输入名称（直接回车取消并返回菜单）："
  read -rp "名称：" unsanitized_client
  # 若用户未输入名称，取消本操作并返回主菜单
  [ -z "$unsanitized_client" ] && { echo; echo "已取消，返回主菜单。"; return 1; }
  # 处理客户端名称（过滤特殊字符、限制长度）
  set_client_name
  # 验证名称合法性（非空且未重复）
...
# 检查是否存在已配置的客户端（无客户端时报错）
# [F10] 传参 "menu" 时改为返回码1（由菜单循环接回主菜单），否则保持 exit 1（命令行分支语义不变）
check_clients() {
  num_of_clients=$(grep -c '^# BEGIN_PEER' "$WG_CONF")
...
# [F10] 空输入（直接回车）= 取消本操作并返回（返回码1），不再退出脚本
# [F11] 该函数现由「客户端管理」二级菜单调用，故取消后回的是二级菜单
select_client_to() {
  echo
  echo "请选择要$1的客户端（直接回车取消并返回菜单）："
  show_clients
  read -rp "客户端编号：" client_num
  # 若用户未输入编号，取消本操作并返回「客户端管理」菜单（[F11]）
  [ -z "$client_num" ] && { echo; echo "已取消，返回客户端管理菜单。"; return 1; }
...
# [F10] read 遇 EOF 时视为 "N"（安全默认：不删除），避免死循环
confirm_remove_client() {
  if [ "$assume_yes" != 1 ]; then
    echo
    read -rp "确认删除 $client 吗？[y/N]：" remove || remove=n
    # 验证用户输入（仅接受y/Y/n/N）
...
# [F10] read 遇 EOF 时视为 "N"（安全默认：不卸载），避免死循环
confirm_remove_wg() {
  if [ "$assume_yes" != 1 ]; then
    echo
    read -rp "确认卸载WireGuard吗？[y/N]：" remove || remove=n
    # 验证用户输入（仅接受y/Y/n/N）
    until [[ "$remove" =~ ^[yYnN]*$ ]]; do
      echo "$remove：选择无效。"
      read -rp "确认卸载WireGuard吗？[y/N]：" remove || remove=n
...
wgsetup() {
  ...
  if [[ ! -e "$WG_CONF" ]]; then
    ...
    # 显示安装完成信息
    finish_setup
  # 分支7：已安装WireGuard（存在配置文件），进入管理菜单
  else
    show_header
    # [F9] 老版本安装的系统同样补上 wg 名称显示增强（幂等，重复执行无副作用）
    install_wg_show_names
    # [F12] 旧版升级上来的服务器补开网络优化（v1.4.3 修复）：
    # 状态检测读的是配置里的 fq_codel 行，而 C8 之前装的旧版写的是 fq ——
    # 升级 wg.sh 后菜单会显示「关闭」，"默认开启"落空。这里补齐：
    # 用户未做过选择（无 marker）且当前无优化 → 自动开启一次并明示；
    # 用户明确关闭过（有 off marker）则不动。幂等，只影响本机 sysctl/tc，
    # 不碰 wg0.conf、不重启服务、不影响在线隧道。
    if ! net_opt_enabled && [ ! -f "$NET_OPT_MARKER" ]; then
      net_opt_on
      echo "※ 已按默认开启网络优化（fq_codel 队列 + netdev_max_backlog=10000，即时生效）。"
      echo "  如需关闭，请进菜单 3) 网络优化 操作。"
    fi
    ...
    while true; do
      ...
      case "$option" in
      1)
        # 选项1：添加新客户端（名称输入为空 = 取消，直接返回菜单）
        if ! enter_client_name; then
          continue
        fi
        select_dns
        new_client add_client
        ...
        ;;
      2)
        # 选项2：管理已有客户端（[F11] 进入二级菜单：列出/删除/显示QR/返回上一级；
        # 二级菜单里选「4) 返回上一级」或 EOF 退出后回到本主菜单循环）
        manage_clients
        ;;
      3)
      ...
      4)
        ...
        if [[ "$remove" =~ ^[yY]$ ]]; then
          ...
          print_wg_removed
          echo
          echo "已退出。重新运行本脚本可重新安装WireGuard。"
          exit 0
        else
          print_wg_removal_aborted
          press_enter_to_menu
